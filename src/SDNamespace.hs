...
handle_def_nat (DefineNat ntid ds source target opts shape) =
    ...
    where
        boundary_not_globelet_error = "The source and target in the definition of of the natural transformation "
...
sdns_chain_lookup_func eid opt = sdns_lookup_add eid functor opt func_opt_lens 
...
sdns_chain_lookup_nat eid opt =  sdns_lookup_add eid nat_trans opt nat_opt_lens 
...
cfe_msg_helper (n1, str1, n2, str2) = concat [str1, " at position ", show n1, " and ", str2, " at position ", show n2]
...
read_functor_line list = do items <- list_ce_to_funcs list
...
list_ce_to_funcs list = let list_with_pos = zip [0..] list
                        ...
                        in do funcs <- lift $ mapM (\(CompElement cid opts) -> runMaybeT $ sdns_chain_lookup_func cid opts) 
                              ...
                              case null lookup_errors of 
                                   False -> throwError $ LookupFunctorError $ map (\(_x,_y,_z)-> (_x,_y)) lookup_errors
                                   True -> return $ map (\(x,y,Just z) -> (x,y,z)) lookup_good
...
compose_funcs list = do comp <- withExcept mExcept $ func_compose_with_error $ map (view _3) list
...
list_ce_to_nt list m = do found_list <- lift $ mapM (runExceptT.ce_to_nt) list
                          let lookup_errs = filter (isLeft.snd) $ zip [0..] found_list
                          case null lookup_errs of 
                               True -> return $ map (\(Right r) -> r) found_list
                               False -> throwError $ LookupNatTransError m $ map (\(n,Left eid)->(n,eid)) lookup_errs
...
impute_missing_nat line nats func = impute' nats (func_to_single_list func) 0
    where
        ...
        impute' [] _ _ = return []
        impute' (Nothing:_) [] n = throwError $ ImputationError line n
        impute' (Nothing:ns) (f:fs) n = impute' ns fs (n+1) >>= (\x -> return ((identityNaturalTransformation f):x))
        impute' ((Just nat):ns) fs n = impute' ns (drop (nat_source_length nat) fs) (n+1) >>= (\x -> return (nat:x))
...
get_first_fff ((SDDrawNat ces):_) = do elems <- list_ce_to_nt ces 0
...
read_nat_trans sdls = do (f,_) <- get_first_fff sdls
...
handle_sdc (DefineCat cid ds) = handle_def_cat (DefineCat cid ds)
handle_sdc (DefineFunc fid ds source_id target_id opts) = handle_def_fun (DefineFunc fid ds source_id target_id opts)
handle_sdc (DefineNat ntid ds source target opts shape) = handle_def_nat (DefineNat ntid ds source target opts shape)
handle_sdc (DrawNat fn opts ces) = handle_draw_nat (DrawNat fn opts ces)
